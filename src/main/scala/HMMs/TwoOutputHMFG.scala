...
class TwoOutputHMFG(
  ...
  def packageParameters = TwoOutputHMFGParameters(
  ...
  def initialPartialCounts = TwoOutputHMFGPartialCounts(
      0D,
      HashMap(
  ...
  def reestimate( corpus: List[List[ObservedStatePair]] ) = {
    ...
    corpus.foreach{ string =>
      ...
      transitionCounts.keySet.foreach{ q =>

        corpusInitialStateCounts(q) = Maths.sumLogProb(
            corpusInitialStateCounts(q),
            initialStateCounts(q)
        )

        corpusFinalStateCounts(q) = Maths.sumLogProb(
            corpusFinalStateCounts(q),
            finalStateCounts(q)
        )

        transitionCounts(q).keySet.foreach{ qTo =>
          corpusTransitionCounts(q)(qTo) = Maths.sumLogProb(
              corpusTransitionCounts(q)(qTo),
              transitionCounts(q)(qTo)
          )
        }

        emissionCountsA(q).keySet.foreach{ obsA =>
          corpusEmissionCountsA(q)(obsA) = Maths.sumLogProb(
              corpusEmissionCountsA(q)(obsA),
              emissionCountsA(q)(obsA)
          )
        }

        emissionCountsB(q).keySet.foreach{ obsB =>
          corpusEmissionCountsB(q)(obsB) = Maths.sumLogProb(
              corpusEmissionCountsB(q)(obsB),
              emissionCountsB(q)(obsB)
          )
        }
      }
    ...
    val finalStateProbs = HashMap(
      corpusFinalStateCounts.keySet.map{ qTo =>
        qTo -> 
          ( corpusFinalStateCounts( qTo ) )
      }.toSeq:_*
    )
