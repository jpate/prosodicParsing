...
import ProsodicParsing.HMMs.CoupledHMFG
...
object CoupledHMFGChunker {
  def main( args:Array[String]) {
    ...
    val manager = actorOf(
        new CoupledHMFG(hiddenStates,observationTypes,"Master")
        ...
        var hmms = (0 to (numHMFGs-1)).map{ n =>
          actorOf(
            new CoupledHMFG( hiddenStates, observationTypes.toSet, n.toString )
        ...
        val viterbiHMM =
          actorOf(
            new CoupledHMFG( hiddenStates, observationTypes.toSet, "viterbi" )
              with HMMActor[HiddenStatePair,ObservedStatePair]
          )
