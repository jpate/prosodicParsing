...
import ProsodicParsing.HMMs.TwoOutputHMFG
...
object TwoOutputChunker {
  def main( args:Array[String]) {
    ...
    val manager = actorOf(
        new TwoOutputHMFG(chunkingStates,observationTypes,"Master")
        ...
        var hmms = (0 to (numHMFGs-1)).map{ n =>
          actorOf( new TwoOutputHMFG( chunkingStates, observationTypes.toSet, n.toString )
            with HMMActor[HiddenState,ObservedStatePair]).start
        }.toList
        println( "Made " + hmms.size + " HMFGs")

        val viterbiHMM = actorOf( new TwoOutputHMFG( chunkingStates, observationTypes.toSet, "viterbi" ) with
